...
- **Datasets** – The Stack v2 Python subset for training; HumanEval/MBPP/ClassEval for evaluation. `seq_length` must be divisible by `chunk_size`; `<|pad|>` must exist in the tokenizer (enforced).
...
- **Hyperparameters** – `rollout_length` 64, `budget_limit` 3–5x, PPO clip 0.2, value clip 0.2, PPO epochs 4, PPO minibatch 32, entropy bonus 0.01, grad clip 1.0, PID gains (0.1, 0.01, 0.05). Adjust per scaling study.
