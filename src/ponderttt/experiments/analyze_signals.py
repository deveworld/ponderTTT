...
def fit_forward(model, input_ids, attention_mask, position_ids):
    ...
    loss_update = cross_entropy_loss(
        out_update["logits"][:, :-1], input_ids[:, 1:], valid_mask
    )
...
def main():
    ...
    if ckpt_path:
        ...
        try:
            ...
            print(
                "Running with initialized model (expect ~0 advantage if untrained TTT)."
            )
    else:
        print(
            "No checkpoint provided/found. TTT layer is untrained (initialized to identity)."
        )
    ...
    for i, batch in enumerate(tqdm(data_iter, total=args.num_batches)):
        ...
        for c_idx in range(num_chunks):
            ...
            metrics["ttt_improvement"] = (
                metrics["ttt_loss_initial"] - metrics["ttt_loss_final"]
            )
