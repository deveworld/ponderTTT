...
def permute_within_chunks(input_ids: jax.Array, seed: int) -> jax.Array:
    ...
    perms = jax.random.permutation(key, jnp.arange(L), axis=0, independent=True)
    # If B > 1, apply same perm or different? Paper implied global shuffle or per-chunk.
    # We'll do per-sequence permutation for simplicity, but same across batch if seed fixed.
    # Actually, random.permutation independent=True generates one perm.
    # Let's map over batch.
...
@nnx.jit(static_argnames=("causal_k",))
def jit_ttt_forward_with_stats(
    ...
    Args:
        causal_k: Causal mask diagonal offset (0 = include diagonal, -1 = exclude).
    """
    # SKIP path (no TTT)
    out_skip = model(
        input_ids,
    ...
    # We must temporarily override causal_k in config if needed
    # NNX models are mutable, but JIT requires caution.
    # However, TTTLayer uses config.causal_k.
    # We can pass a hacked config or rely on the fact that we can't easily change it inside JIT 
    # unless the model accepts it as arg.
    # TTTModel.__call__ doesn't accept causal_k.
    # Wait, TTTLayer implementation uses `self.config.causal_k`.
    # We should handle this outside JIT or update model config before calling.
    # Actually, let's assume the caller sets the config correctly BEFORE calling this JIT function.
    # So we remove causal_k arg here and rely on model state.
